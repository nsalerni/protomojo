...
def assert_invalid_outputs(
    ...
    assert "**Result: 79/79 checks passed.**" not in markdown

    html = html_verdict(validation, "test time")
    assert '<span class="score failing">invalid</span>' in html
    assert '<span class="score">79/79</span>' not in html
    ...
    assert "invalid result set" in markdown
    assert "**Result: 79/79 checks passed.**" not in markdown
    assert "80/79" not in markdown
    assert '<span class="score failing">invalid</span>' in html
    assert '<span class="score">79/79</span>' not in html
    assert "80/79" not in html
    assert badge["color"] == "red"
...
def test_result_registry() -> None:
    ...
    assert validation.passed_count == 79
    assert validation.expected_count == 79
    assert "**Result: 79/79 checks passed.**" in markdown_verdict(
        validation, "test time"
    )
    assert '<span class="score">79/79</span>' in html_verdict(validation, "test time")
    ...
    assert "**Result: 79/79 checks passed.**" in markdown
    assert '<span class="score">79/79</span>' in html
    ...
    assert failed_validation.passed_count == 78
    assert "**Result: 78/79 checks passed.**" in markdown_verdict(
        failed_validation, "test time"
    )
    assert '<span class="score failing">78/79</span>' in html_verdict(
        failed_validation, "test time"
    )
    assert (
    ...
    assert "**Result: 78/79 checks passed.**" in markdown
    assert '<span class="score failing">78/79</span>' in html
    assert badge["color"] == "red"

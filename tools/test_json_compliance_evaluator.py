...
def assert_invalid_outputs(
    ...
    assert "**Result: 41/41 checks passed.**" not in markdown

    html = html_verdict(validation, "test time")
    assert '<span class="score failing">invalid</span>' in html
    assert '<span class="score">41/41</span>' not in html
    ...
    assert "invalid result set" in markdown
    assert "**Result: 41/41 checks passed.**" not in markdown
    assert "42/41" not in markdown
    assert '<span class="score failing">invalid</span>' in html
    assert '<span class="score">41/41</span>' not in html
    assert "42/41" not in html
    assert badge["color"] == "red"
...
def test_result_registry() -> None:
    ...
    assert validation.passed_count == 41
    assert validation.expected_count == 41
    assert "**Result: 41/41 checks passed.**" in markdown_verdict(
        validation, "test time"
    )
    assert '<span class="score">41/41</span>' in html_verdict(validation, "test time")
    ...
    assert "**Result: 41/41 checks passed.**" in markdown
    assert '<span class="score">41/41</span>' in html
    ...
    assert failed_validation.passed_count == 40
    assert "**Result: 40/41 checks passed.**" in markdown_verdict(
        failed_validation, "test time"
    )
    assert '<span class="score failing">40/41</span>' in html_verdict(
        failed_validation, "test time"
    )
    assert (
    ...
    assert "**Result: 40/41 checks passed.**" in markdown
    assert '<span class="score failing">40/41</span>' in html
    assert badge["color"] == "red"

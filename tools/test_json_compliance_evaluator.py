...
def assert_invalid_outputs(
    ...
    assert "**Result: 75/75 checks passed.**" not in markdown

    html = html_verdict(validation, "test time")
    assert '<span class="score failing">invalid</span>' in html
    assert '<span class="score">75/75</span>' not in html
    ...
    assert "invalid result set" in markdown
    assert "**Result: 75/75 checks passed.**" not in markdown
    assert "76/75" not in markdown
    assert '<span class="score failing">invalid</span>' in html
    assert '<span class="score">75/75</span>' not in html
    assert "76/75" not in html
    assert badge["color"] == "red"
...
def test_result_registry() -> None:
    ...
    assert validation.passed_count == 75
    assert validation.expected_count == 75
    assert "**Result: 75/75 checks passed.**" in markdown_verdict(
        validation, "test time"
    )
    assert '<span class="score">75/75</span>' in html_verdict(validation, "test time")
    ...
    assert "**Result: 75/75 checks passed.**" in markdown
    assert '<span class="score">75/75</span>' in html
    ...
    assert failed_validation.passed_count == 74
    assert "**Result: 74/75 checks passed.**" in markdown_verdict(
        failed_validation, "test time"
    )
    assert '<span class="score failing">74/75</span>' in html_verdict(
        failed_validation, "test time"
    )
    assert (
    ...
    assert "**Result: 74/75 checks passed.**" in markdown
    assert '<span class="score failing">74/75</span>' in html
    assert badge["color"] == "red"

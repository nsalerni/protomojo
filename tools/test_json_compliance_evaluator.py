...
def assert_invalid_outputs(
    ...
    assert "**Result: 46/46 checks passed.**" not in markdown

    html = html_verdict(validation, "test time")
    assert '<span class="score failing">invalid</span>' in html
    assert '<span class="score">46/46</span>' not in html
    ...
    assert "invalid result set" in markdown
    assert "**Result: 46/46 checks passed.**" not in markdown
    assert "47/46" not in markdown
    assert '<span class="score failing">invalid</span>' in html
    assert '<span class="score">46/46</span>' not in html
    assert "47/46" not in html
    assert badge["color"] == "red"
...
def test_result_registry() -> None:
    ...
    assert validation.passed_count == 46
    assert validation.expected_count == 46
    assert "**Result: 46/46 checks passed.**" in markdown_verdict(
        validation, "test time"
    )
    assert '<span class="score">46/46</span>' in html_verdict(validation, "test time")
    ...
    assert "**Result: 46/46 checks passed.**" in markdown
    assert '<span class="score">46/46</span>' in html
    ...
    assert failed_validation.passed_count == 45
    assert "**Result: 45/46 checks passed.**" in markdown_verdict(
        failed_validation, "test time"
    )
    assert '<span class="score failing">45/46</span>' in html_verdict(
        failed_validation, "test time"
    )
    assert (
    ...
    assert "**Result: 45/46 checks passed.**" in markdown
    assert '<span class="score failing">45/46</span>' in html
    assert badge["color"] == "red"

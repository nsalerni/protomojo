...
def assert_invalid_outputs(
    ...
    assert "**Result: 15/15 checks passed.**" not in markdown

    html = html_verdict(validation, "test time")
    assert '<span class="score failing">invalid</span>' in html
    assert '<span class="score">15/15</span>' not in html
    ...
    assert "invalid result set" in markdown
    assert "**Result: 15/15 checks passed.**" not in markdown
    assert "16/15" not in markdown
    assert '<span class="score failing">invalid</span>' in html
    assert '<span class="score">15/15</span>' not in html
    assert "16/15" not in html
    assert badge["color"] == "red"
...
def test_result_registry() -> None:
    ...
    assert validation.expected_count == 15
    assert "**Result: 15/15 checks passed.**" in markdown_verdict(
        validation, "test time"
    )
    assert '<span class="score">15/15</span>' in html_verdict(validation, "test time")
    ...
    assert "**Result: 15/15 checks passed.**" in markdown
    assert '<span class="score">15/15</span>' in html
    ...
    assert failed_validation.passed_count == 14
    assert "**Result: 14/15 checks passed.**" in markdown_verdict(
        failed_validation, "test time"
    )
    assert '<span class="score failing">14/15</span>' in html_verdict(
        failed_validation, "test time"
    )
    assert (
    ...
    assert "**Result: 14/15 checks passed.**" in markdown
    assert '<span class="score failing">14/15</span>' in html
    assert badge["color"] == "red"

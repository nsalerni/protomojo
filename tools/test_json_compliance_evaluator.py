...
def assert_invalid_outputs(
    ...
    assert "**Result: 36/36 checks passed.**" not in markdown

    html = html_verdict(validation, "test time")
    assert '<span class="score failing">invalid</span>' in html
    assert '<span class="score">36/36</span>' not in html
    ...
    assert "invalid result set" in markdown
    assert "**Result: 36/36 checks passed.**" not in markdown
    assert "37/36" not in markdown
    assert '<span class="score failing">invalid</span>' in html
    assert '<span class="score">36/36</span>' not in html
    assert "37/36" not in html
    assert badge["color"] == "red"
...
def test_result_registry() -> None:
    ...
    assert validation.passed_count == 36
    assert validation.expected_count == 36
    assert "**Result: 36/36 checks passed.**" in markdown_verdict(
        validation, "test time"
    )
    assert '<span class="score">36/36</span>' in html_verdict(validation, "test time")
    ...
    assert "**Result: 36/36 checks passed.**" in markdown
    assert '<span class="score">36/36</span>' in html
    ...
    assert failed_validation.passed_count == 35
    assert "**Result: 35/36 checks passed.**" in markdown_verdict(
        failed_validation, "test time"
    )
    assert '<span class="score failing">35/36</span>' in html_verdict(
        failed_validation, "test time"
    )
    assert (
    ...
    assert "**Result: 35/36 checks passed.**" in markdown
    assert '<span class="score failing">35/36</span>' in html
    assert badge["color"] == "red"

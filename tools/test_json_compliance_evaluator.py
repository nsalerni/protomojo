...
def assert_invalid_outputs(
    ...
    assert "**Result: 17/17 checks passed.**" not in markdown

    html = html_verdict(validation, "test time")
    assert '<span class="score failing">invalid</span>' in html
    assert '<span class="score">17/17</span>' not in html
    ...
    assert "invalid result set" in markdown
    assert "**Result: 17/17 checks passed.**" not in markdown
    assert "18/17" not in markdown
    assert '<span class="score failing">invalid</span>' in html
    assert '<span class="score">17/17</span>' not in html
    assert "18/17" not in html
    assert badge["color"] == "red"
...
def test_result_registry() -> None:
    ...
    assert validation.passed_count == 17
    assert validation.expected_count == 17
    assert "**Result: 17/17 checks passed.**" in markdown_verdict(
        validation, "test time"
    )
    assert '<span class="score">17/17</span>' in html_verdict(validation, "test time")
    ...
    assert "**Result: 17/17 checks passed.**" in markdown
    assert '<span class="score">17/17</span>' in html
    ...
    assert failed_validation.passed_count == 16
    assert "**Result: 16/17 checks passed.**" in markdown_verdict(
        failed_validation, "test time"
    )
    assert '<span class="score failing">16/17</span>' in html_verdict(
        failed_validation, "test time"
    )
    assert (
    ...
    assert "**Result: 16/17 checks passed.**" in markdown
    assert '<span class="score failing">16/17</span>' in html
    assert badge["color"] == "red"

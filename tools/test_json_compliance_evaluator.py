...
def assert_invalid_outputs(
    ...
    assert "**Result: 51/51 checks passed.**" not in markdown

    html = html_verdict(validation, "test time")
    assert '<span class="score failing">invalid</span>' in html
    assert '<span class="score">51/51</span>' not in html
    ...
    assert "invalid result set" in markdown
    assert "**Result: 51/51 checks passed.**" not in markdown
    assert "52/51" not in markdown
    assert '<span class="score failing">invalid</span>' in html
    assert '<span class="score">51/51</span>' not in html
    assert "52/51" not in html
    assert badge["color"] == "red"
...
def test_result_registry() -> None:
    ...
    assert validation.passed_count == 51
    assert validation.expected_count == 51
    assert "**Result: 51/51 checks passed.**" in markdown_verdict(
        validation, "test time"
    )
    assert '<span class="score">51/51</span>' in html_verdict(validation, "test time")
    ...
    assert "**Result: 51/51 checks passed.**" in markdown
    assert '<span class="score">51/51</span>' in html
    ...
    assert failed_validation.passed_count == 50
    assert "**Result: 50/51 checks passed.**" in markdown_verdict(
        failed_validation, "test time"
    )
    assert '<span class="score failing">50/51</span>' in html_verdict(
        failed_validation, "test time"
    )
    assert (
    ...
    assert "**Result: 50/51 checks passed.**" in markdown
    assert '<span class="score failing">50/51</span>' in html
    assert badge["color"] == "red"

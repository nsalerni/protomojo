...
def assert_invalid_outputs(
    ...
    assert "**Result: 70/70 checks passed.**" not in markdown

    html = html_verdict(validation, "test time")
    assert '<span class="score failing">invalid</span>' in html
    assert '<span class="score">70/70</span>' not in html
    ...
    assert "invalid result set" in markdown
    assert "**Result: 70/70 checks passed.**" not in markdown
    assert "71/70" not in markdown
    assert '<span class="score failing">invalid</span>' in html
    assert '<span class="score">70/70</span>' not in html
    assert "71/70" not in html
    assert badge["color"] == "red"
...
def test_result_registry() -> None:
    ...
    assert validation.passed_count == 70
    assert validation.expected_count == 70
    assert "**Result: 70/70 checks passed.**" in markdown_verdict(
        validation, "test time"
    )
    assert '<span class="score">70/70</span>' in html_verdict(validation, "test time")
    ...
    assert "**Result: 70/70 checks passed.**" in markdown
    assert '<span class="score">70/70</span>' in html
    ...
    assert failed_validation.passed_count == 69
    assert "**Result: 69/70 checks passed.**" in markdown_verdict(
        failed_validation, "test time"
    )
    assert '<span class="score failing">69/70</span>' in html_verdict(
        failed_validation, "test time"
    )
    assert (
    ...
    assert "**Result: 69/70 checks passed.**" in markdown
    assert '<span class="score failing">69/70</span>' in html
    assert badge["color"] == "red"

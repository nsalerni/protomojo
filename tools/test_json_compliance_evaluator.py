...
def assert_invalid_outputs(
    ...
    assert "**Result: 22/22 checks passed.**" not in markdown

    html = html_verdict(validation, "test time")
    assert '<span class="score failing">invalid</span>' in html
    assert '<span class="score">22/22</span>' not in html
    ...
    assert "invalid result set" in markdown
    assert "**Result: 22/22 checks passed.**" not in markdown
    assert "23/22" not in markdown
    assert '<span class="score failing">invalid</span>' in html
    assert '<span class="score">22/22</span>' not in html
    assert "23/22" not in html
    assert badge["color"] == "red"
...
def test_result_registry() -> None:
    ...
    assert validation.passed_count == 22
    assert validation.expected_count == 22
    assert "**Result: 22/22 checks passed.**" in markdown_verdict(
        validation, "test time"
    )
    assert '<span class="score">22/22</span>' in html_verdict(validation, "test time")
    ...
    assert "**Result: 22/22 checks passed.**" in markdown
    assert '<span class="score">22/22</span>' in html
    ...
    assert failed_validation.passed_count == 21
    assert "**Result: 21/22 checks passed.**" in markdown_verdict(
        failed_validation, "test time"
    )
    assert '<span class="score failing">21/22</span>' in html_verdict(
        failed_validation, "test time"
    )
    assert (
    ...
    assert "**Result: 21/22 checks passed.**" in markdown
    assert '<span class="score failing">21/22</span>' in html
    assert badge["color"] == "red"
